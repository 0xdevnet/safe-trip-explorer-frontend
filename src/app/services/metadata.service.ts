...
export class MetadataService {
  constructor(private http: HttpClient, notify: NotifierService, private url: Url) {
  ...
  getDataAPI() {
    let url: string = this.url.metadata_url + this.address;
    const headers = {};
    this.setLoading(true);
    this.http
      .get(url, { headers, responseType: 'json', observe: 'body' })
      .subscribe({
        next: (data) => {
